...
def process_song_file(cur, filepath):
    """Process to fill the artist table with the artist data
    
    Keyword arguments:
    cur -- the cursor set on the main
    filepath -- the path to the file with the data on JSON format
    """
...
def process_log_file(cur, filepath):
    ...
    # load user table
    user_df = df.sort_values(by='ts', ascending=False).loc[:,['userId','firstName', 
                                                              'lastName', 'gender',
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = (pd.to_datetime(df.ts[index].item()), int(df.userId[index]), df.level[index], songid, artistid, 
...
def process_data(cur, conn, filepath, func):
    """Process of executing the function defi
    
    Keyword arguments:
    cur -- the cursor set on the main
    conn -- the connection configuration to the source
    filepath -- the path to the file with the data on JSON format
    func -- function previous defined
    """
